...
const
  Computer_training = () => {
    ...
    const [valueRatting2, setValueRatting2] = useState([]);
    ...
    const val = useSelector((state) => state.valueReducer.val);
    ...
    const trainingOption = async (v) => {
      await ApiGet("/training_option/subject/" + v)
        .then((res) => {
          setMeeting(
            res.data?.data?.user_batch?.length > 0 &&
            res?.data?.data?.user_batch[0]?.shortMeetingURL
          );
          // console.log("res", res.data.data.user_batch[0].shortMeetingURL);
          setOptions(res.data.data);
          setPdf(
            res.data?.data?.documents[0]?.document_image
              ? res.data?.data?.documents[0]?.document_image
              : []
          );
          setIsLoading(true);
      ...
      if (token === undefined) {
        ...
        ApiGet("/training_type/subject/" + v._id)
          .then(async (res) => {
            ...
            await res?.data?.data[0]?.training.map((v, i) => {
              console.log("vasd", v);
              if (v?.isSelected?.length > 0) {
                setState(v?.name)
              }
            })
            setIsLoading(true);
    ...
    useEffect(() => {
      ApiGet("/feedback_question")
        .then((res) => {
          ...
          // setFirstReview(firstHalf);
          // setSecondReview(secondHalf);
    ...
    const handleSubmit = (e, id, que, index) => {
      //
      //
      ...
      if (arrayIncludesInObj(valueRatting, "questionId", id)) {
        ...
        let final = [
          ...filterData,
          { questionId: id, ans: e.target.value, question: que, index },
        ];
    ...
    const submitRatting = () => {
      ...
      const sortList = valueRatting
        .sort((a, b) => a.index - b.index)
        .map((exemple, index, array) => exemple);
      if (valueRatting.length === review.length) {
        ApiPost("/feedback_answer/add", {
          subjectId: Index,
          question: sortList,
        })
          .then((res) => {
        ...
        const ref = window.document.querySelector(".modal");
        ref.scrollTo({ top: 0, left: 0, behavior: "smooth" });
        setAlert(true);
        setRattingSubmit(false);
      }
    };
    console.log("valueRatting", valueRatting);
    return (
      <div className="bgColor">
        ...
        <ReviewModal
          ...
          alerts={alerts}
          valueRatting={valueRatting}
          review={review}
          setRattingModal={setRattingModal}
          setDisable={setDisable}
          submitRatting={submitRatting}
          handleSubmit={handleSubmit}
          rattingSubmit={rattingSubmit}
          myRef={myRef}
        />
      </div>
    );
